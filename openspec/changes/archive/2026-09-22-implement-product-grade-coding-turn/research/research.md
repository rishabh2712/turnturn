...
| [Tool UI](https://github.com/assistant-ui/tool-ui) | `49a8702` | `../turnturn-references/tool-ui/apps/www/app/docs/overview/content.mdx`; `apps/www/app/docs/design-guidelines/content.mdx`; `apps/www/app/docs/approval-card/content.mdx`; `apps/www/components/tool-ui/approval-card/approval-card.tsx`; `apps/www/components/tool-ui/code-diff/code-diff.tsx` (latter paths relative to the Tool UI clone) | Purpose-built tool result/approval displays; information/state/decision roles; stable action IDs; durable receipt after a choice. | A generic JSON viewer as the primary surface, or a visual diff that implies an edit is still pending after it was applied. |
...
Tool UI's decision receipt is useful here because `ToolCallView.approval` already survives as `allowed` or `denied` after the pending `ApprovalRequestItem` disappears. That is a receipt for the choice, **not** evidence that the tool succeeded. Its role/addressability guidance supports using durable `approvalId`/`toolCallId` for both the inline decision and persistent reminder. The compact reminder itself is a turnturn design choice; the reference does not prescribe that placement.
