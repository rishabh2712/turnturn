...
- [x] Wire Stop through the existing `turn.cancel` command, with current-state checks and pending/error handling until the terminal record arrives. Do not add Retry: no retry command or agreed retry semantics exist yet. Record that as a later design choice.
  - 2026-09-22: The selected-conversation controller sends a scoped cancel once, keeps “Stopping…” until a durable terminal record is projected, and refreshes after a stale rejection. Browser-level tests cover accepted response before terminal, duplicate click, and the eventual aborted outcome; a controller test covers `TURN_NOT_RUNNING` without falsely showing success. Real-provider cancellation remains unverified.
- [x] Before placing a second approval surface, define the turn's interaction grammar from the reference evidence: request, provisional activity, typed action/result, blocking decision, durable decision receipt, answer/outcome. Record what each surface owns and how unknown inputs fall back. See `design.md` “Interaction grammar for one coding turn.”
- [x] Give pending approvals one controller operation and one command status shared by inline and persistent affordances. Preserve the currently passing reload behavior: the approval identity must come from the recovered projection, not component-local state.
  - 2026-09-22: `ApprovalCard` is presentational, and `PendingActionBar` stays outside the scrolling transcript. Both use the projected `approvalId` and one controller status. Tests cover simultaneous controls, duplicate prevention, stale resolution, reload, DOM placement when scrolled, failed command and catch-up, and the durable receipt. “Allowed” is a decision receipt, not tool success. Actual browser layout/focus and live-provider approval remain unverified under sections 3 and 4.
- [x] Verify affected tests and reducer issues for any record-producing test; do not alter record sequence numbers to make a test pass.
  - 2026-09-22: `@turnturn/protocol` 26/26 (including the durable-record reducer/writer/reader suites), `@turnturn/chat-client` 111/111, `@turnturn/web` 97/97. Web lint, typecheck, and build are clean, and the build stays under the Shiki bundle budget (177.9 KB / 250.0 KB gzip). No record sequence numbers were changed to pass a test; `reduceEngineState` reports no issues for the new approval-surfaces scenarios.
