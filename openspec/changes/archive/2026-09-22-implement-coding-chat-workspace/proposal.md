...
- **A server-owned provider and model catalog.** Configured Anthropic, LiteLLM, and Ollama connections discover the models available to the server, expose only safe public metadata, and feed a provider-grouped model picker. Credentials, headers, and editable endpoints remain server-only.
