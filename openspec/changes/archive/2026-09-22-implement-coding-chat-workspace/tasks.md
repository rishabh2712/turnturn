...
### Review findings — rich-message and viewport slice, 2026-09-21

Recorded before fixes, per Rishabh's review rule. These are defects in the uncommitted T6/8.5 implementation, not new product scope.

- [x] RF1 `FilePeekPanel` called `fetch` directly even though `ChatTransport.getWorkspaceFile()` already owned that boundary. Fixed by moving request lifecycle into `features/file-preview/WorkspaceFilePeekProvider.tsx`; the panel is now a pure loading/ready/error view. A fake-transport component test makes global `fetch` throw and proves it is never touched.
- [x] RF2 `ConversationViewport` observed only `presentation.timeline.length`, so same-item live deltas did not follow while pinned and initial hydration could open at the top. Fixed by separating `contentRevision` from `itemCount`: every content revision follows only while pinned, only count deltas affect the unread pill, and initial node attachment scrolls to latest. Hook and component tests cover pinned, unpinned, same-item growth, and initial attachment.
- [x] RF3 `CodeBlock` put toolbar/block elements inside an outer `<pre>` and Shiki added a nested `<pre>`. Fixed with a non-`pre` card root, exactly one preformatted element on every path, and scoped code, heading, blockquote, table, and inline-code styles. Structural tests reject nested pre elements and toolbar-inside-pre.
- [x] RF4 `closeUnterminatedFence()` counted fence-looking lines and always appended three characters. Replaced with a delimiter-aware scan that tracks marker type and opening length; tests cover backticks, tildes, longer delimiters, embedded shorter runs, and already-complete fences.
- [x] RF5 `renderMarkdownCached()` trusted a 32-bit FNV digest plus length. Cache entries now retain the exact source and a hit requires source equality. A regression test uses two real same-length FNV-colliding strings and proves their rendered trees cannot cross.
- [x] RF6 The committed D30 catalog declared `ProviderRegistry` and stable connection ids, but indexed all catalog state by `WireKind` and constructed adapters outside the registry. Fixed by making connection id the catalog/state/profile-id key, permitting multiple connections on one wire, and routing adapter creation through `ProviderRegistry`. The regression test creates LiteLLM and direct OpenAI on `openai-chat-completions` and proves separate groups, ids, discovery results, and endpoint-owning adapters.

RF1–RF5 are one review unit because they restore the already-approved D17/D19/D20/D21 boundaries. RF6 is the corresponding contract repair for approved D30. Do not implement D31 or D32 as part of this repair.
...
- [x] D30 amendment review by the user before 5.4c implementation. Approved by Rishabh on 2026-09-19 after a summary of the provider connection, capability-state, refresh, and catalog-drift decisions; implementation of 5.4c.0–5.4c.7 followed and is committed at `eb2de08`.
- [ ] D31 amendment review by the user before its 9.4/9.7 implementation. Read D31 (the pinned approval bar) and the updated "Approvals are resolved in place without losing context" spec scenarios; approve or revise the two-surface (inline card + pinned bar) design, the retirement of the off-screen banner, and its non-interaction with the pin/jump-to-latest mechanism.
- [ ] D32 amendment review by the user before its 6.8 implementation. Read D32 (soft appearance of streaming text) and the new "Streaming assistant text arrives with a subtle transition" spec scenarios; approve or revise the per-chunk-not-per-word scope, the no-replay-on-durable rule, and the rejection of a word-by-word typewriter effect.
- [ ] D33 amendment review by the user before the frontend-boundary task below. Read D33 and approve or revise the five-layer responsibility table, the `ConversationController` extraction, and the rule that leaf views receive callbacks rather than transport infrastructure.
...
### 5B. T4B — Frontend responsibility boundaries (D33)

This is a behavior-preserving architecture task. It follows RF1–RF5 and is not required to repair them. Do not combine it with D31, D32, Markdown features, or a visual redesign.

- [ ] 5B.1 Add import-boundary checks: leaf modules under `components/` cannot import `ChatTransport`, `HttpChatTransport`, `@turnturn/protocol` record/event types, or call browser-global `fetch`; `apps/web` cannot interpret durable/live event type strings. Pin the allowed exceptions while migration is in progress rather than pretending the rule is already true.
- [ ] 5B.2 Extract `features/provider-catalog/useProviderCatalog.ts` from `App.tsx`. It owns catalog loading/refresh state and returns data/actions; `ModelPicker` remains a pure controlled view.
- [ ] 5B.3 Extract `features/conversation/useConversationController.ts`. It subscribes to `ConversationStore`, starts/stops resume, and owns submit, approval resolution, model activation, busy/error/draft/trace state. It returns a stable screen model and action callbacks; it emits no JSX.
- [ ] 5B.4 Extract `screens/ConversationScreen.tsx` and the existing composer/header into focused views. The screen composes presentation objects and controller actions; it does not call the transport or inspect records/events.
- [ ] 5B.5 Finish the file-preview feature boundary begun by RF1: controller owns transport and request lifecycle; `FilePeekPanel` is loading/ready/error markup only. Prove the same feature works with a fake `ChatTransport` and no global `fetch`.
- [ ] 5B.6 Reduce `App.tsx` to provider installation, selected-screen routing, and shell-level error placement. Add an executable import/dependency check and update the component responsibility map in D33 if the implemented names differ.

Acceptance: replacing `HttpChatTransport` with the existing fake requires no view-component change; no leaf component knows an API route, server token, durable record, or live event; `App.tsx` contains no conversation workflow; all existing behavioral tests stay green.
...
- [x] 6.1 Write the containment tests first (D17, D18, security): a `<script>` in assistant text does not execute and is not interpreted; an element with an inline event handler is stripped; a `javascript:` link renders as plain text; an image renders as a link, not an `<img>`; an external link carries `rel="noopener noreferrer nofollow"`. Done in `test/markdown-security.test.tsx`, written and green against `MarkdownMessage` before any Markdown library rendering was wired past a stub; also added `test/tool-output-not-markdown.test.tsx` as the D18 regression (Markdown/HTML-shaped tool stdout and file-read content stay literal text in `<pre>`, never interpreted — `ToolActionCard` was already `<pre>`-only and untouched).
- [x] 6.2 Add `react-markdown@10.1.0`, `remark-gfm@4.0.1`, `rehype-sanitize@6.0.0`, and `shiki@4.4.3` (all current-registry, matching D17's verified versions), and build the Markdown pipeline with a restrictive `rehype-sanitize` schema (`src/lib/markdown/sanitize-schema.ts`, GitHub's default schema plus an http/https/mailto/`turnturn` href allowlist and the two `data-workspace-path`/`data-line` attributes for 6.6) wired into `TurnBlock.tsx` via the new `MarkdownMessage` component. Verify: headings, lists, GFM tables, blockquotes, and inline code render (`test/markdown-rendering.test.tsx`), and 6.1 passes. Also fixed a CSS regression Rishabh found testing this live in the browser — numbered/bulleted list markers were separated from their item text with paragraph-sized gaps (react-markdown wraps loose list items in `<p>`, no scoped CSS collapsed it) — with `.tt-markdown ul/ol/li/li > p` rules at the end of `styles.css` and `test/markdown-list-styling.test.tsx`.
- [x] 6.3 Add the Shiki `code` renderer: lazily loaded, fixed language subset, copy button carrying the block's exact source, and unhighlighted fallback above 2,000 lines or 100 KB or for an unknown language. Done in `src/lib/markdown/shiki-loader.ts` (fine-grained `shiki/core` + `shiki/engine/javascript` + per-language `shiki/langs/*.mjs` imports so Vite code-splits each grammar separately, and only the core/engine/theme load up front — a specific language loads only once a block actually needs it) and `src/components/markdown/CodeBlock.tsx`. Verified in `test/markdown-code-block.test.tsx`: highlighting appears after Shiki resolves, the copy button copies the exact pre-highlight source text (asserted via a mocked `navigator.clipboard.writeText`), and an unrecognized language or an oversized (2,500-line) block renders as plain text without throwing.
- [x] 6.4 Handle streaming Markdown: auto-close an unterminated fence for display only, skip highlighting until the block closes, memoize on the text prefix. Done: `src/lib/markdown/streaming-fence.ts#closeUnterminatedFence` derives a display-only string inside `MarkdownMessage` (never writes back to the `content`/`streaming` props owned by the transport store) and preserves the opener's marker and length; `CodeBlock` skips Shiki entirely via `useMarkdownStreaming()` while a message is still streaming. Verified in `test/markdown-streaming.test.tsx`: incomplete backtick and tilde fences, longer delimiters, embedded shorter runs, and complete fences all retain received text without mutating input, and no Shiki highlighting occurs while streaming.
- [x] 6.5 Add the content-hash LRU for parsed output and the cheap has-Markdown pre-check. Done: `src/lib/markdown/lru-cache.ts` (generic bounded LRU), `src/lib/markdown/hash.ts` (FNV-1a lookup key), `src/lib/markdown/render-cache.ts#renderMarkdownCached` (stores and verifies the exact source beside each rendered value, so hash collisions remain correct), and `src/lib/markdown/has-markdown-syntax.ts` for the plain-reply fast path. Verified in `test/markdown-cache.test.tsx`: LRU eviction, parse reuse, distinct-content parsing, and a real same-length FNV collision.
- [x] 6.6 Implement file references (D19): the `remark-workspace-paths` plugin over text nodes only, the `turnturn://file/…` anchor with `data-workspace-path`/`data-line`, the `FileReferenceProvider` with an injected `openFile`, and the in-app peek fed through `ChatTransport.getWorkspaceFile()`. The feature controller owns transport/request state; `FilePeekPanel` only renders literal loading/ready/error state. Also added the explicit `turnturn:` URL allowlist. Verified in `test/markdown-file-references.test.tsx`: references exclude inline/fenced code, clicks use the injected host action, and file peek works through a fake transport while global `fetch` is forbidden.
- [x] 6.7 Add the strict `Content-Security-Policy` to the built `index.html` and a bundle-size budget check for the Shiki subset. Done: `index.html` sets `default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self'; font-src 'self'; connect-src 'self'; object-src 'none'; base-uri 'none'; form-action 'none'` via `<meta http-equiv>` (reasoning inline in the HTML comment above it — `style-src 'unsafe-inline'` is kept solely because Shiki emits per-token inline `color` styles, not attacker-controlled markup; `frame-ancestors` is omitted because browsers ignore it entirely from a `<meta>` tag, confirmed live in the dev server console, and belongs on a server response header instead). Verified live against the Vite dev server (which also confirmed no separate HMR-websocket CSP allowance is needed — `connect-src 'self'` covers a same-origin `ws:` upgrade): no CSP violations in the console after removing `frame-ancestors`. Bundle budget in `apps/web/scripts/bundle-budget.mjs` (250 KB gzip across the whole lazily-loaded Shiki chunk set — core + JS regex engine + one theme + all 16 language grammars — vs. a measured ~178 KB on 2026-09-20, chosen with ~35% headroom over the measurement rather than per-chunk, since a full-subset load is the realistic worst case for one conversation), run from `pnpm build` via `scripts/check-bundle-budget.mjs`. Verified in `test/bundle-budget.test.ts`: passes against the real `dist/assets`, fails against a synthetic oversized fixture, and ignores non-Shiki chunks.
- [ ] 6.8 Implement the soft streaming-text transition per D32, gated on the D32 design-gate item above. CSS-only opacity/blur-in transition on newly appended live-text chunks; full opacity immediately on the durable message; `prefers-reduced-motion: reduce` disables the transition entirely rather than shortening it. Verify: a chunk appended mid-stream carries the transition class/style and an already-rendered chunk does not get re-animated; the durable-superseding render has no transition; a reduced-motion media query match renders with no transition applied.
...
- [x] 8.5 Implement scroll behaviour: pinned within 48px of the bottom, unpin on user scroll, "Jump to latest" with an arrived-since count, click to scroll and re-pin, and never move the viewport while unpinned. Verify on the pinned/unpinned state and the count rather than on pixels — `happy-dom` has no layout, and the test should say so.
  - Done: the pin/unpin/jump-to-latest decision lives in a standalone hook, `apps/web/src/lib/scroll-pin.ts` (`isWithinPinZone` pure function plus `useScrollPin`), rather than embedded in a single component — D31 documents a second future caller (the pinned approval bar's "View" action, not built here) that needs to re-arm auto-follow through this same mechanism, so `pin()` is exported separately from `jumpToLatest()` for that reuse. `apps/web/src/components/viewport/ConversationViewport.tsx` wraps the scrollable transcript with the hook and renders `apps/web/src/components/viewport/JumpToLatest.tsx` (a pill showing the arrived-since count); wired into `App.tsx` in place of the old plain `<div className="tt-transcript">`, with `itemCount` driven by the projected `presentation.timeline.length`.
  - Verified per the task's own instruction: `apps/web/test/scroll-pin.test.ts` asserts `isWithinPinZone` directly on plain numbers (including exactly-at-48px and one-pixel-past), then drives `useScrollPin` with a fake element whose `scrollTop`/`scrollHeight`/`clientHeight` are set as writable properties — covering starts-pinned, auto-scroll while pinned, unpin on scroll, no viewport movement plus accumulating count while unpinned (multiple arrivals, not just the last), `jumpToLatest` scrolling and re-pinning with count reset, auto-scroll resuming after re-pin, and `pin()` re-arming without moving the element. `apps/web/test/conversation-viewport.test.tsx` covers the same scenarios at the component level via `data-pinned`/`data-new-item-count` and the rendered pill text, never a pixel value. No test asserts real layout; both files' header comments say why, per the repo gotcha "`happy-dom` has no layout."
  - `pnpm --filter @turnturn/web {lint,build,typecheck,test}` and root `pnpm {lint,build,typecheck,test}` are all green (81 web tests passing, including the 15 new ones added by this task across the two new test files).
  - Not built here, per the task's explicit boundary: `<PendingApprovalBar>` and its "View" re-pin call (D31/9.4/9.7) and the D32 streaming fade-in — both remain gated on their own pending design-gate items above.
...
- [ ] 9.4 Superseded by D31 (see "Implementer handoff — 9.4 pinned approval bar" below). Do not build the conditional off-screen banner; build `<PendingApprovalBar>` instead.
...
- [ ] 9.7 Implement `<PendingApprovalBar>` per D31 (see handoff below), gated on the D31 design-gate item above.

Acceptance: an approval can be answered without losing the context that produced it; every failure has a stated cause and, where one exists, a recovery; a restart mid-turn is legible; a pending approval is visible without the reader having to notice it scrolled away.

#### Implementer handoff — 9.4/9.7 pinned approval bar (D31)

Read `design.md` D16, D31, and the "Scrolling" interaction-detail paragraph, plus the updated "Approvals are resolved in place without losing context" spec scenarios, before starting.

- Add `apps/web/src/components/approval/PendingApprovalBar.tsx`: always mounted in `<MainColumn>` between `<ConversationHeader>` and `<ConversationViewport>`; renders nothing when the active session has no pending approval; otherwise renders one summary line built with the same headline vocabulary as the tool-summary group algorithm (D15/4.6), an Allow control, a Deny control, and a "View" control.
- Both Allow/Deny on the bar dispatch the exact same `approval.resolve` client action already used by `ApprovalCard` (9.2) — do not fork the submission path. Both surfaces subscribe to the same store slice, so resolving from either disables both without extra plumbing.
- "View" scrolls the transcript to the inline `ApprovalCard`'s DOM node and briefly highlights it; this scroll must go through whatever re-pin mechanism 8.5 implements for "Jump to latest", so the viewport ends up pinned at that position rather than in an inconsistent state.
- Delete `apps/web/src/components/status/ApprovalBanner.*` (or wherever 9.4 would have lived) if it was already started; do not build a conditional off-screen-only variant.
- `Escape` handling (8.6) must not resolve an approval regardless of whether focus is on the bar or the card — extend the existing test rather than writing a parallel one.
- Tests: the bar appears the instant `approval.requested` projects, independent of scroll position (a test that never scrolls should still see it); resolving from the bar updates the inline card and vice versa; the bar disappears on resolution and on the cancelled-path (9.3); the bar never changes the viewport's pinned/unpinned state on its own appearance or disappearance; `aria-live="assertive"` is present; tab order reaches the bar right after the header.
